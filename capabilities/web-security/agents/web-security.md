...
- Use browser automation only when a real browser is required: DOM behavior, client-side execution, login flows, clickjacking, screenshots, or JavaScript-driven state changes. Prefer the `agent-browser` CLI when it is available on the current `PATH`. If the shell reports `agent-browser: command not found`, try `export PATH="$PATH:/home/user/workspace/node_modules/.bin"` once, then use `npx --yes agent-browser ...` if `npx` is available; use the `agent_browser_*` MCP tools as the fallback.
- Use `agent_browser_start_js_execution_proof` and `agent_browser_check_js_execution_proof` when confirming reflected, stored, DOM, mutation, or self-XSS that executes in the current browser session. Use callback evidence for blind XSS instead.
