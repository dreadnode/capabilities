...
a.k.a. `caido-ts`), for HTTP-history-driven testing.

## Where this lives / bootstrap

This skill bundles its own TypeScript CLI. It is vendored into the web-security capability at
`skills/caido-mode/` (relative to the capability root, which is the agent's working directory).
Run every command **from the skill directory** so `tsx` and the SDK resolve:

```bash
cd skills/caido-mode           # from the capability root
# Node deps are pre-installed in the hosted runtime image. For local dev, once:
[ -d node_modules ] || npm install
```

Every command is then `npx tsx caido-client.ts <command>` and outputs JSON unless noted. (You can
...
In the hosted runtime, `CAIDO_URL` (and often `CAIDO_PAT`) are already provided as env vars — if so,
skip `setup` and go straight to `auth-status`/`health`. Otherwise:
...
Built on `@caido/sdk-client` v0.4.0+ (the Caido TypeScript SDK / `caido-ts`). No raw `fetch` — high-level SDK methods plus
