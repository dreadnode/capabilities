...
class TestFailuresStayNonFatal:
    def test_optional_vendor_downloads_do_not_abort_the_run(self) -> None:
        # Burp, Caido and the browser binaries are not redistributable, so a
        # disconnected deployment will not have them. That must degrade the
        # capability, not take the rest of the tooling down with it.
        for marker in (
