...
MODEL_ALIASES: dict[str, str] = {
    # Groq
    "groq llama": "groq/meta-llama/llama-4-maverick-17b-128e-instruct",
    "groq": "groq/meta-llama/llama-4-maverick-17b-128e-instruct",
    "llama": "groq/meta-llama/llama-4-maverick-17b-128e-instruct",
    ...
    "groq 17b": "groq/meta-llama/llama-4-maverick-17b-128e-instruct",
    "groq 17b maverick": "groq/meta-llama/llama-4-maverick-17b-128e-instruct",
...
def _build_imports(attacks: list[dict], transforms: list[dict], has_scorers: bool) -> str:
    ...
    if has_scorers:
        lines.append("from dreadnode.airt.scorers import llm_judge")
...
{assessment_kwargs}
    ...
    for i, (label, transforms, transform_names) in enumerate(STUDIES, 1):
        ...
        try:
            ...
            assessment.add_result(ar)
            await assessment.upload_result(ar)
            ...
            if ar.finished_trials:
                ...
                candidate_preview = best.candidate[:150] + "..." if len(best.candidate) > 150 else best.candidate
                response_preview = best.response[:150] + "..." if len(best.response) > 150 else best.response
                print(f"  Best candidate: {{candidate_preview}}")
            ...
            traceback.print_exc()
            sys.stdout.flush()
            # Upload a failed result so the attack run doesn't stay in pending
            try:
                ...
                assessment.add_result(failed_ar)
                await assessment.upload_result(failed_ar)
            except Exception:
                pass
            continue
...
_CAMPAIGN_ATTACK_BLOCK = '''\
    ...
    try:
        ...
        _{var}_result = await _{var}_study.console()
        ...
        assessment.add_result({var})
        await assessment.upload_result({var})
        ...
        try:
            ...
            assessment.add_result(_failed)
            await assessment.upload_result(_failed)
        except Exception:
            pass
'''
...
def _generate_transform_study(config: dict) -> str:
    ...
    params.append("airt_target_model=TARGET_MODEL")
    params.append("airt_transforms=transform_names")
    attack_params = ",\n                ".join(params)
    ...
    return "\n".join([imports, configure, cfg, "", tgt, body])
...
'''.format(kwargs=assessment_kwargs)
    ...
    parts = [imports, configure, cfg, "", tgt, campaign_header]
...
def _generate_category_attack(config: dict) -> str:
    ...
    params.append("airt_target_model=TARGET_MODEL")
    if transforms:
        params.append("airt_transforms={}".format(repr([t["resolved_name"] for t in transforms])))
    attack_params = ",\n                        ".join(params)
...
def generate_category_attack(params: dict) -> dict:
    ...
    result_lines = [
        ...
        "",
        "Execute:",
        "cd ~/workspace/airt && python3 workflows/{}".format(filename),
        "",
        "Config:",
        "  Mode: Category Sweep",
