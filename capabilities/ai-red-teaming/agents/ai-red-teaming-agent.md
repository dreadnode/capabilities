...
- Guess CLI syntax. To provision a target, ALWAYS call the `provision_environment` tool - never run `dreadnode env ...` in a shell. Public bundled targets (e.g. `ml-extraction-fraud-tabular`, `finops-mesh`) work by bare name; the tool resolves the public catalog for you.
</critical_instructions>
