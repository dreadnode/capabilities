...
graphs use the BBOT event envelope. Treat `.data` as the canonical event value;
supporting fields may include `id`, `uuid`, `type`, `tags`, `scope_distance`,
`module`, and `scan`. Do not assume convenience fields such as `.name`,
`.address`, `.version`, or `.provider`. Relationship names vary by module and
record type; inspect them before writing analysis queries. Common relationship
types include `A`, `CNAME`, `httpx`, `portscan`, and `nuclei`.
...
**Recent scans:**
```cypher
MATCH (s:SCAN) RETURN s.data, s.id ORDER BY s.id DESC LIMIT 10
```
