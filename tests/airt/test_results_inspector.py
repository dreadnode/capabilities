...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / "dreadnode" / "ai-red-teaming" / "scripts" / "results_inspector.py"
...
class TestInspectResults:
    ...
    def test_reads_analytics_file(self, temp_results_dir) -> None:
        ...
        analytics_dir.mkdir()
        (analytics_dir / "test_analytics.json").write_text(json.dumps({
            "asr": 0.45,
            "risk_score": 6.5,
