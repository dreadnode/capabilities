    ...
    def response_for(path: str, query: dict[str, list[str]]) -> object:
        ...
        if path == "/shodan/host/203.0.113.10":
            return {
                ...
                "asn": "AS64500",
                "isp": "Example ISP",
                "data": [{"port": 9200, "product": "Elasticsearch", "version": "7.10.0", "data": "banner"}],
            }
        if path == "/shodan/host/count":
            return {"total": 1, "facets": {"product": [{"value": "Elasticsearch", "count": 1}]}}
        if path == "/dns/resolve":
        ...
        if path == "/shodan/protocols":
            return {"http": "HTTP"}
        if path == "/api-info":
            return {"plan": "mock", "query_credits": 100, "scan_credits": 0, "unlocked": True}
        if path == "/shodan/query/search":
            return {"total": 1, "matches": [{"title": "Databases", "query": "port:9200"}]}
...
def test_http_shodan_client_uses_mock_base_url_and_api_key():
    ...
    try:
        ...
        assert all(request_query.get("key") == ["test-key"] for _, request_query in MockShodanHandler.requests)
    finally:
        server.shutdown()
...
def test_shodan_tool_formatting_with_mock_client(monkeypatch):
    ...
    try:
        ...
        search = json.loads(shodan_mcp.shodan_host_search('org:"Target Corp"', facets="port"))
