...
def _get_impacket_script_path() -> Path | None:
    """
    Auto-discover the impacket scripts directory.

    Tries multiple common locations:
    1. Global PATH (pipx install): scripts available as commands
    2. pip-installed: site-packages/impacket/examples/
    3. apt-installed: /usr/share/doc/python3-impacket/examples/

    Returns:
        Path to impacket examples directory, or None if globally accessible.
    """
    # Check if scripts are globally accessible (pipx installation)
    if shutil.which("secretsdump.py") is not None:
        return None  # Scripts are in PATH, no directory needed
...
class Impacket(Toolset):
    ...
    script_path: Path | None = Config(default=g_default_impacket_path)
    """Directory containing the impacket scripts, or None if globally accessible."""
    ...
    def _build_script_command(self, script_name: str, args: list[str]) -> list[str]:
        ...
        if self.script_path is None:
            # Scripts are globally accessible (pipx installation)
            return [script_name, *args]
        # Scripts in a specific directory
        return [sys.executable, str(self.script_path / script_name), *args]
